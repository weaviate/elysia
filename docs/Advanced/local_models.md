...
You can choose to disable just for the base model (e.g. the decision agent) or just the complex model (which some tools will use). Custom tools can also make use of the base/complex models also via the custom DSPy Module [`ElysiaPrompt`](https://weaviate.github.io/elysia/Reference/Util/#elysia.util.modules.ElysiaPrompt).
